...
static const unsigned short CRC7_TABLE[256] = {
...
static uint16_t _crc7(const void *buffer, size_t length) {
    const uint8_t *b = buffer;
    unsigned short crc = 0;
    for (size_t i = 0; i < length; i++) {
        crc = (crc << 8) ^ CRC7_TABLE[((crc >> 8) ^ b[i]) & 0x00FF];
    }
    return crc;
}
...
static int _read_bytes(void *_config, uint8_t *buffer, uint32_t length) {
    ...
    if (config->enable_crc) {
        uint32_t crc_result = _crc7(buffer, length);
...
static int _read(void *_config, uint8_t *buffer, uint32_t length) {
    ...
    if (config->enable_crc) {
        uint16_t crc_result = _crc7(buffer, length);
        if (crc_result != crc) {
...
static uint8_t _write(void *_config, const uint8_t *buffer, uint8_t token, uint32_t length) {
    ...
    if (config->enable_crc) {
        crc = _crc7(buffer, length);
    }

    // write the checksum CRC-7
