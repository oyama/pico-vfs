...
Block devices and file systems are integrated into the POSIX file API by the VFS layer. Users can perform operations like `open()`, `read()`, and `write()` as usual, and also use higher-level functions such as `fopen()`, `fprintf()`, and `fgets()`. The available POSIX and C standard file APIs are listed in [STANDARDS.md](STANDARDS.md). File mounting and formatting APIs not defined in POSIX are provided under the `fs_*()` namespace.
